...
const outputPath = join(__dirname, '..', 'src', 'css', 'tokens', 'colors.css')
...
let count = 0
const lines = ['@layer zui.base {', '  :root {']

for (const [name, shades] of Object.entries(colors)) {
  const label = name.charAt(0).toUpperCase() + name.slice(1)
  lines.push(`    /* ${label} */`)
  for (const [shade, value] of Object.entries(shades)) {
    lines.push(`    --zui-${name}-${shade}: ${value};`)
    count++
  }
  lines.push('')
}

lines.push('    /* Black & White */')
lines.push('    --zui-black: #000;')
lines.push('    --zui-white: #fff;')
count += 2
...
console.log(`✅ Generated ${count} color tokens → ${outputPath}`)
