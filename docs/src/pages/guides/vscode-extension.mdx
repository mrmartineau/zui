...
The extension reads the copy of `@mrmartineau/zui` installed in your workspace's `node_modules` and builds its class and token lists from that package's CSS. Completions always match the ZUI version your project actually depends on — no stale suggestions after you pin an older release, and new classes appear as soon as you upgrade. When no installed copy can be found (for example with `zui.enable: "always"` in a project that uses ZUI via CDN), the extension falls back to a bundled snapshot of the latest release.
...
The extension re-evaluates whenever any `package.json` in the workspace changes, so adding, removing, or upgrading ZUI as a dependency toggles and refreshes completions without a reload.
