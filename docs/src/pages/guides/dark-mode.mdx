...
title: Dark Mode
description: ZUI uses light-dark() for all colour tokens, respecting prefers-color-scheme automatically. Override at runtime with the utils package.
---
...
### `nextColorScheme(current?)`

Returns the next scheme in the cycle: `light` → `dark` → `system` → `light` …

If `current` is omitted it reads from `localStorage` automatically.

```ts
import { getColorScheme, setColorScheme, nextColorScheme } from '@mrmartineau/zui/utils'

// Advance to the next scheme and apply it
...
import { nextColorScheme } from '@mrmartineau/zui/utils'

function ThemeToggle() {
  const { scheme, set } = useColorScheme()

  return (
    <button onClick={() => set(nextColorScheme(scheme))}>
      {scheme === 'dark' ? '🌙 Dark' : scheme === 'light' ? '☀️ Light' : '💻 System'}
    </button>
  )
}
```
