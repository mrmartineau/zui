...
- `variant` supports `surface` and `underline`
- use the same `variant` on `TabsList` and its `TabsTrigger` children
...
- React, Astro, Solid, Svelte, and Vue are scaffolded on top of the shared tabs core