...
`AppShell` is the top-level layout for application-style UIs. It owns the grid (sidebar + header over main), the collapse/expand state, and the desktop/mobile mode switch. Children compose the sidebar regions, header, and main content area.
...
The sidebar collapses to an icon rail on desktop. Mark elements inside the sidebar that should disappear when collapsed with the `data-collapsed-hide` attribute — typically labels next to icons.
...
<Demo
  ...
  <aside class="zui-app-shell-sidebar" popover="auto" aria-label="Sidebar">
    <div class="zui-app-shell-sidebar-header">
      ...
      <span data-collapsed-hide>Acme</span>
    </div>
    <div class="zui-app-shell-sidebar-body">
      <a href="#"><i class="ph ph-house"></i><span data-collapsed-hide>Home</span></a>
    </div>
  ...
  return (
    <AppShell shortcut>
      <AppShellSidebar>
        ...
        <AppShellSidebarBody>
          <a href="#"><i className="ph ph-house" /><span data-collapsed-hide>Home</span></a>
        </AppShellSidebarBody>
        <AppShellSidebarFooter>
...
## Collapse-aware hiding

Wrap any element inside the sidebar with `data-collapsed-hide` to hide it (with a sequenced fade) when the sidebar is in its collapsed rail state. The attribute has no effect in mobile drawer mode — there the sidebar is either fully open or closed.
...
The mobile breakpoint is enforced by a container query, so AppShell works correctly even when embedded inside a narrower parent (split views, iframes, etc.).
...
.my-app .zui-app-shell {
  ...
  --zui-app-shell-sidebar-rail-width: 4rem;
  --zui-app-shell-header-height: 3.5rem;
  --zui-app-shell-main-padding: var(--space-lg);
}
```

To change the mobile breakpoint, re-author the `@container zui-app-shell (max-width: ...)` rule in your own stylesheet — container queries cannot read CSS custom properties.
