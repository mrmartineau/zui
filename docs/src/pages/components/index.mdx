...
Every component exposes a set of CSS custom properties (variables) that control its appearance — things like colours, border radius, padding, and shadow. You can override these without touching the source CSS, and without needing to eject or copy any styles.
...
Target the component class in your own stylesheet. Because component styles live in `zui.components`, any rule outside a `@layer` will already win — no `!important` needed:
...
Each component's page lists its available custom properties and their defaults.
